...
static void gattSetAdvertParams(bool fast_connection)
{
    ...
    if((g_lightapp_data.gatt_data.enable_white_list == TRUE))
    {
         ...
         */
        if(LsAddWhiteListDevice(&g_lightapp_data.gatt_data.bonded_bd_addr)!=
            ls_err_none)
        {
...
{
    if (g_lightapp_data.gatt_data.idle_tid == tid)
    {
        if (g_lightapp_data.state == app_state_idle)
        {
    ...
    if(g_lightapp_data.gatt_data.app_tid == tid)
    {
        ...
        switch(g_lightapp_data.state)
        {
            case app_state_fast_advertising:
            {
                ...
                TimerDelete(g_lightapp_data.gatt_data.idle_tid);

                g_lightapp_data.gatt_data.idle_tid = TimerCreate(
...
extern void GattTriggerFastAdverts(void)
{
    ...
    if(g_lightapp_data.gatt_data.advert_timer_value)
    {
