 ...
 *
 *  DESCRIPTION
 *      Implementation of the CSR Mesh bridge GATT-related routines
 *
 *****************************************************************************/
...
static void gattSetAdvertParams(bool fast_connection)
{
    ...
    if((g_bridgeapp_data.gatt_data.enable_white_list == TRUE))
    {
...
static void gattAdvertTimerHandler(timer_id tid)
{
    ...
    if(g_bridgeapp_data.gatt_data.app_tid == tid)
    {
        ...
        switch(g_bridgeapp_data.state)
        {
            case app_state_fast_advertising:
            {
                if((g_bridgeapp_data.gatt_data.enable_white_list == TRUE))
                {
                     ...
                     * list.
                     */
                    g_bridgeapp_data.gatt_data.advert_timer_value =
...
extern void GattStartAdverts(bool fast_connection)
{
    ...
    if(g_bridgeapp_data.gatt_data.advert_timer_value)
    {
        TimerDelete(g_bridgeapp_data.gatt_data.app_tid);

        /* Start advertisement timer  */
        g_bridgeapp_data.gatt_data.app_tid = TimerCreate(
                            g_bridgeapp_data.gatt_data.advert_timer_value,
