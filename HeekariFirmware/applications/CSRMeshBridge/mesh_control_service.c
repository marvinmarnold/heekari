...
extern void MeshControlHandleAccessWrite(GATT_ACCESS_IND_T *p_ind)
{
    ...
    switch(p_ind->handle)
    {
        ...
        case HANDLE_MTL_CP2_CLIENT_CONFIG:
        {
            ...
            Nvm_Write((uint16*)&g_mesh_svc_data.mtl_cp_ccd,
                       sizeof(gatt_client_config),
                       g_mesh_svc_data.nvm_offset);
        }
        break;
...
/*----------------------------------------------------------------------------*
 *  NAME
 *      MeshControlReadDataFromNVM
 *
 *  DESCRIPTION
 *      This function is used to read Mesh Control service
 *      specific data stored in NVM
 *
 *  RETURNS
 *      Nothing.
 *
...
{
    g_mesh_svc_data.nvm_offset = *p_offset;

    /* Read NVM only if devices are bonded */
    if(AppIsDeviceBonded())
    {
        /* Read Mesh Control characteristic client configuration descriptor */
        Nvm_Read((uint16*)&g_mesh_svc_data.mtl_cp_ccd,
                    sizeof(g_mesh_svc_data.mtl_cp_ccd),
                    *p_offset +
                    NVM_MESH_CONTROL_MTL_CP_CCD_OFFSET);
    }

    /* Increment the offset by the number of words of NVM memory required
     * by the Mesh Control service.
     */
    *p_offset += NVM_MESH_CONTROL_SERVICE_MEMORY_WORDS;
}
