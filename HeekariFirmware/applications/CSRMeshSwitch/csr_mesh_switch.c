 ...
 *
 *  DESCRIPTION
 *      This file defines a CSRmesh switch application 
 *
 *****************************************************************************/
...
/* CSR Mesh device UUID size */
#define DEVICE_UUID_SIZE_WORDS         (8)

/* CSR Mesh Authorization Code Size in Words */
...
/* NVM offset for Switch destination address */
#define NVM_OFFSET_SWITCH_STATE        (NVM_OFFSET_DEVICE_AUTHCODE + \
                                        DEVICE_AUTHCODE_SIZE_IN_WORDS)

#define NVM_OFFSET_DST_ID_LIST        (NVM_OFFSET_SWITCH_STATE + \
                                        sizeof(uint16))

#define NVM_BEARER_DATA_OFFSET        (NVM_OFFSET_DST_ID_LIST + \
                                        sizeof(dst_id_list))
...
/* CSR Mesh switch application data instance */
CSRMESH_SWITCH_DATA_T g_switchapp_data;

/* The target group/device id list, the switch will send control messages to */
uint16 dst_id_list[IOT_NUM_SWITCH_INSTANCES];
uint8  level = 0;
bool   incButtonState = KEY_RELEASED;
bool   decButtonState = KEY_RELEASED;
bool   onButtonState  = KEY_RELEASED;
uint16 oneSecTimerId  = 0;
uint16 switch_uuid[DEVICE_UUID_SIZE_WORDS] =
                  {0x22e4,0xb12c,0x5042,0x11e3, 0x9618,0xce3f,0x5508,0xacd9};

/* CSR Mesh Device Authorization Code */
uint16 switchAuthCode[DEVICE_AUTHCODE_SIZE_IN_WORDS] =
 ...
 *============================================================================*/
static void smSwitchDataInit(void);
...
static void smSwitchDeviceIdAdvertTimeoutHandler(timer_id tid);
...
static void handleButtonDebounce(timer_id tid)
{
    ...
    if( tid == g_switchapp_data.debounce_tid)
    {
        ...
        if ((PioGet(SW3_PIO) == FALSE) && (incButtonState == KEY_RELEASED))
        {
            ...
            if (level < (MAX_LEVEL - LEVEL_STEP_SIZE))
                level += LEVEL_STEP_SIZE;
            else
                level = MAX_LEVEL;
        ...
        if ((PioGet(SW4_PIO) == FALSE) && (onButtonState == KEY_RELEASED))
        {
            ...
            PowerSetState(dst_id_list[0], POWER_STATE_ON, FALSE);
        }
        else if ((PioGet(SW4_PIO) == TRUE) && (onButtonState == KEY_PRESSED))
        {
            ...
            PowerSetState(dst_id_list[0], POWER_STATE_OFF, FALSE);
        }

        /* Send Light Command and Create One Second Timer when flag is set */
        if (startOneSecTimer)
        {
            LightSetLevel(dst_id_list[0], level, FALSE);
    ...
    else if (tid == oneSecTimerId)
    {
        ...
        {
            if (level < (MAX_LEVEL - (5*LEVEL_STEP_SIZE)))
                level += (5*LEVEL_STEP_SIZE);
            else
                level = MAX_LEVEL;
    ...
    if (update_nvm)
    {
        NvmWrite((uint16 *)&level, sizeof(uint16), NVM_OFFSET_SWITCH_STATE);
    }
}
...
/*-----------------------------------------------------------------------------*
 *  NAME
 *      smSwitchDeviceIdAdvertTimeoutHandler
 *
 *  DESCRIPTION
...
static void smSwitchDeviceIdAdvertTimeoutHandler(timer_id tid)
{
    if(tid == g_switchapp_data.mesh_device_id_advert_tid)
    {
        ...
        {
            if (g_switchapp_data.state != app_state_fast_advertising)
            {
            ...
            g_switchapp_data.mesh_device_id_advert_tid = TimerCreate(
                                         DEVICE_ID_ADVERT_TIMER_ID, TRUE,
                                         smSwitchDeviceIdAdvertTimeoutHandler);
        }
        else
        {
 ...
 *
 *  DESCRIPTION
 *      This function is called to initialise CSR Mesh switch application
 *      data structure.
 *
 *  RETURNS
 *      Nothing.
...
static void smSwitchDataInit(void)
{
    ...
    TimerDelete(g_switchapp_data.gatt_data.app_tid);
    g_switchapp_data.gatt_data.app_tid = TIMER_INVALID;

    TimerDelete(g_switchapp_data.gatt_data.con_param_update_tid);
    g_switchapp_data.gatt_data.con_param_update_tid = TIMER_INVALID;
    g_switchapp_data.gatt_data.cpu_timer_value = 0;

    g_switchapp_data.gatt_data.st_ucid = GATT_INVALID_UCID;

    g_switchapp_data.gatt_data.enable_white_list = FALSE;

    g_switchapp_data.gatt_data.advert_timer_value = 0;

    /* Reset the connection parameter variables. */
    g_switchapp_data.gatt_data.conn_interval = 0;
    g_switchapp_data.gatt_data.conn_latency = 0;
    g_switchapp_data.gatt_data.conn_timeout = 0;
 ...
 *
 *  DESCRIPTION
 *      This function starts timer to send CSR Mesh Association Messages
 *      and also gives visual indication that switch is not associated.
 *
 *  RETURNS/MODIFIES
 *      Nothing
 *
...
static void smAppInitiateAssociation(void)
{
    ...
    g_switchapp_data.mesh_device_id_advert_tid =
                    TimerCreate(DEVICE_ID_ADVERT_TIMER_ID,
                                TRUE,
                                smSwitchDeviceIdAdvertTimeoutHandler);
}
...
static void readPersistentStore(void)
{
    ...
    if(nvm_sanity == NVM_SANITY_MAGIC)
    {
        ...
        Nvm_Read((uint16*)&g_switchapp_data.gatt_data.bonded,
                  sizeof(g_switchapp_data.gatt_data.bonded),
                  NVM_OFFSET_BONDED_FLAG);

        if(g_switchapp_data.gatt_data.bonded)
        {
            /* Bonded Host Typed BD Address will only be stored if bonded flag
             * is set to TRUE. Read last bonded device address.
             */
            Nvm_Read((uint16*)&g_switchapp_data.gatt_data.bonded_bd_addr,
                     sizeof(TYPED_BD_ADDR_T),
            ...
            if(GattIsAddressResolvableRandom(
                                &g_switchapp_data.gatt_data.bonded_bd_addr))
            {
                Nvm_Read(g_switchapp_data.gatt_data.central_device_irk.irk,
                         MAX_WORDS_IRK,
                         NVM_OFFSET_SM_IRK);
            }
        }
         ...
         * bonded device.
         */
        Nvm_Read(&g_switchapp_data.gatt_data.diversifier,
                 sizeof(g_switchapp_data.gatt_data.diversifier),
                 NVM_OFFSET_SM_DIV);
        ...
        /* Read Switch State Data : Level */
        Nvm_Read((uint16 *)&level, sizeof(uint16),
                 NVM_OFFSET_SWITCH_STATE);

        /* Read assigned Destination device IDs */
        Nvm_Read((uint16 *)dst_id_list, sizeof(dst_id_list),
                 NVM_OFFSET_DST_ID_LIST);

        /* Read Bearer Model Data from NVM */
        Nvm_Read((uint16 *)&g_switchapp_data.bearer_data,
          ...
          */
    {
        ...
        if (cskey_flags & RANDOM_UUID_ENABLE_MASK)
        {
            ...
            for( i = 0 ; i < 8 ; i++)
            {
                switch_uuid[i] = Random16();
            }
        }

        /* Update Bearer Model Data from CSKey flags for the first time. */
        g_switchapp_data.bearer_data.promiscuous       = FALSE;
        g_switchapp_data.bearer_data.bearerEnabled     = BLE_BEARER_MASK;
        g_switchapp_data.bearer_data.bearerRelayActive = 0x0000;

        if (cskey_flags & RELAY_ENABLE_MASK)
        {
            g_switchapp_data.bearer_data.bearerRelayActive |= BLE_BEARER_MASK;
        }

        if (cskey_flags & BRIDGE_ENABLE_MASK)
        {
            g_switchapp_data.bearer_data.bearerEnabled     |= BLE_GATT_SERVER_BEARER_MASK;
            g_switchapp_data.bearer_data.bearerRelayActive |= BLE_GATT_SERVER_BEARER_MASK;
        }
        ...
        /* Write to NVM */
        Nvm_Write(switch_uuid, DEVICE_UUID_SIZE_WORDS,
                  NVM_OFFSET_DEVICE_UUID);

        /* Write Authorization Code to NVM */
        Nvm_Write(switchAuthCode, DEVICE_AUTHCODE_SIZE_IN_WORDS,
                  NVM_OFFSET_DEVICE_AUTHCODE);

        /* The device will not be bonded as it is coming up for the first
         * time
         */
        g_switchapp_data.gatt_data.bonded = FALSE;

        /* Write bonded status to NVM */
        Nvm_Write((uint16*)&g_switchapp_data.gatt_data.bonded,
                  sizeof(g_switchapp_data.gatt_data.bonded),
                  NVM_OFFSET_BONDED_FLAG);
         ...
         */
        g_switchapp_data.gatt_data.diversifier = 0;

        /* Write the same to NVM. */
        Nvm_Write(&g_switchapp_data.gatt_data.diversifier,
                  sizeof(g_switchapp_data.gatt_data.diversifier),
                  NVM_OFFSET_SM_DIV);
        ...
        NvmWrite((uint16 *)&g_switchapp_data.assoc_state,
                 sizeof(g_switchapp_data.assoc_state),
                 NVM_OFFSET_ASSOCIATION_STATE);

        /* Write Switch State to NVM */
        NvmWrite((uint16 *)&level, sizeof(uint16),
                 NVM_OFFSET_SWITCH_STATE);
    ...
    Nvm_Read(switch_uuid, DEVICE_UUID_SIZE_WORDS,
                       NVM_OFFSET_DEVICE_UUID);

    /* Read Authorization Code from NVM */
    Nvm_Read(switchAuthCode, DEVICE_AUTHCODE_SIZE_IN_WORDS,
             NVM_OFFSET_DEVICE_AUTHCODE);
...
static void requestConnParamUpdate(timer_id tid)
{
    ...
    if(g_switchapp_data.gatt_data.con_param_update_tid == tid)
    {
        ...
        switch(g_switchapp_data.state)
        {
            ...
            case app_state_connected:
            {
                 ...
                 * requests
                 */
                ++ g_switchapp_data.gatt_data.num_conn_update_req;

                /* If it is first or second request, preferred connection
                 * parameters should be request
                 */
                if(g_switchapp_data.gatt_data.num_conn_update_req == 1 ||
                   g_switchapp_data.gatt_data.num_conn_update_req == 2)
                {
                 ...
                 * should be requested.
                 */
                else if(g_switchapp_data.gatt_data.num_conn_update_req == 3 ||
                        g_switchapp_data.gatt_data.num_conn_update_req == 4)
                {
                    app_pref_conn_param.con_max_interval =
                ...
                if(LsConnectionParamUpdateReq(&g_switchapp_data.gatt_data.con_bd_addr,
...
static void handleGapCppTimerExpiry(timer_id tid)
{
    if(g_switchapp_data.gatt_data.con_param_update_tid == tid)
    {
        g_switchapp_data.gatt_data.con_param_update_tid =
                           TimerCreate(TGAP_CPC_PERIOD, TRUE,
                                       requestConnParamUpdate);
        g_switchapp_data.gatt_data.cpu_timer_value = TGAP_CPC_PERIOD;
    }
}
...
{
        if(g_switchapp_data.gatt_data.bonded &&
            (!GattIsAddressResolvableRandom(&g_switchapp_data.gatt_data.bonded_bd_addr)))
        {
            /* If the device is bonded and bonded device address is not
             * resolvable random, configure White list with the Bonded
             * host address
             */
            if(LsAddWhiteListDevice(&g_switchapp_data.gatt_data.bonded_bd_addr)!=
                ls_err_none)
            {
    ...
    switch(g_switchapp_data.state)
    {
        case app_state_connected:
        {
            ...
            if((p_event_data->keys)->keys_present & (1 << SM_KEY_TYPE_DIV))
            {
                 ...
                 */
                g_switchapp_data.gatt_data.diversifier =
             ...
             */
            if(GattIsAddressResolvableRandom(
                                &g_switchapp_data.gatt_data.con_bd_addr) &&
               ((p_event_data->keys)->keys_present & (1 << SM_KEY_TYPE_ID)))
            {
                MemCopy(g_switchapp_data.gatt_data.central_device_irk.irk,
                 ...
                 */
                Nvm_Write(g_switchapp_data.gatt_data.central_device_irk.irk,
                          MAX_WORDS_IRK,
                          NVM_OFFSET_SM_IRK);
            }
...
static void handleSignalSmDivApproveInd(SM_DIV_APPROVE_IND_T *p_event_data)
{
    ...
    switch(g_switchapp_data.state)
    {
        ...
        case app_state_connected:
        {
            ...
            {
                if(g_switchapp_data.gatt_data.diversifier == p_event_data->div)
                {
...
static void handleSignalGattCancelConnectCfm(void)
{
    ...
    switch(g_switchapp_data.state)
    {
        ...
        case app_state_slow_advertising:
            /* There is no idle state, device advertises for ever */
        break;
                                     ...
                                     LM_EV_CONNECTION_COMPLETE_T *p_event_data)
{
    ...
    g_switchapp_data.gatt_data.conn_interval = p_event_data->data.conn_interval;
    g_switchapp_data.gatt_data.conn_latency  = p_event_data->data.conn_latency;
    g_switchapp_data.gatt_data.conn_timeout  =
...
static void handleSignalGattConnectCfm(GATT_CONNECT_CFM_T* p_event_data)
{
    ...
    switch(g_switchapp_data.state)
    {
        ...
        case app_state_slow_advertising:
        {
            ...
            {
                /* Store received UCID */
                g_switchapp_data.gatt_data.st_ucid = p_event_data->cid;

                /* Store connected BD Address */
                g_switchapp_data.gatt_data.con_bd_addr = p_event_data->bd_addr;

                if((g_switchapp_data.gatt_data.bonded) &&
                   (GattIsAddressResolvableRandom(
                            &g_switchapp_data.gatt_data.bonded_bd_addr)) &&
                   (SMPrivacyMatchAddress(&p_event_data->bd_addr,
                           g_switchapp_data.gatt_data.central_device_irk.irk,
                ...
                else
                {
                    ...
                    CsrMeshHandleDataInConnection(
                                    g_switchapp_data.gatt_data.st_ucid,
                                    g_switchapp_data.gatt_data.conn_interval);
                     ...
                     */
                    if(g_switchapp_data.gatt_data.bonded && 
                       (!MemCmp(&g_switchapp_data.gatt_data.bonded_bd_addr, 
                                &g_switchapp_data.gatt_data.con_bd_addr,
                          sizeof(g_switchapp_data.gatt_data.bonded_bd_addr))))
                    {
                        GattOnConnection();
                    }
#endif /* ENABLE_GATT_OTA_SERVICE */
                     ...
                     * with Android 4.4
                     *
                     *  SMRequestSecurityLevel(&g_switchapp_data.gatt_data.con_bd_addr);
                     */
                     ...
                     */
                    if((g_switchapp_data.gatt_data.con_param_update_tid ==
                                                            TIMER_INVALID) &&
                       (g_switchapp_data.gatt_data.conn_interval <
                                                 PREFERRED_MIN_CON_INTERVAL ||
                        g_switchapp_data.gatt_data.conn_interval >
                        ...
                        || g_switchapp_data.gatt_data.conn_latency <
                                                 PREFERRED_SLAVE_LATENCY
#endif
                       )
                      )
                    {
                        ...
                        g_switchapp_data.gatt_data.num_conn_update_req = 0;
                        ...
                        g_switchapp_data.gatt_data.con_param_update_tid =
                                          TimerCreate(TGAP_CPP_PERIOD, TRUE,
                                                      handleGapCppTimerExpiry);
                        g_switchapp_data.gatt_data.cpu_timer_value =
                                                            TGAP_CPP_PERIOD;
                    }
    ...
    switch(g_switchapp_data.state)
    {
        case app_state_connected:
        {
            if(p_event_data->status == sys_status_success)
            {
                 ...
                 */
                g_switchapp_data.gatt_data.bonded = TRUE;
                g_switchapp_data.gatt_data.bonded_bd_addr =
                ...
                Nvm_Write((uint16*)&g_switchapp_data.gatt_data.bonded_bd_addr,
                          sizeof(TYPED_BD_ADDR_T),
                          NVM_OFFSET_BONDED_ADDR);
                                   ...
                                   LM_EV_CONNECTION_UPDATE_T* p_event_data)
{
    switch(g_switchapp_data.state)
    {
        ...
        case app_state_disconnecting:
        {
            ...
            CsrMeshHandleDataInConnection(g_switchapp_data.gatt_data.st_ucid,
                                       g_switchapp_data.gatt_data.conn_interval);

            DEBUG_STR("Parameter Update Complete: ");
            DEBUG_U16(g_switchapp_data.gatt_data.conn_interval);
            DEBUG_STR("\r\n");
        }
        break;
                                 ...
                                 LS_CONNECTION_PARAM_UPDATE_IND_T *p_event_data)
{
    ...
    switch(g_switchapp_data.state)
    {
        case app_state_connected:
        {
            ...
            if(g_switchapp_data.gatt_data.conn_interval <
                                                PREFERRED_MIN_CON_INTERVAL ||
               g_switchapp_data.gatt_data.conn_interval >
                                                PREFERRED_MAX_CON_INTERVAL
#if PREFERRED_SLAVE_LATENCY
...
static void handleSignalGattAccessInd(GATT_ACCESS_IND_T *p_event_data)
{
    ...
    switch(g_switchapp_data.state)
    {
        case app_state_connected:
        {
             ...
             * timer
             */
             if(g_switchapp_data.gatt_data.cpu_timer_value == TGAP_CPC_PERIOD &&
                g_switchapp_data.gatt_data.con_param_update_tid != TIMER_INVALID)
             {
                TimerDelete(g_switchapp_data.gatt_data.con_param_update_tid);
                g_switchapp_data.gatt_data.con_param_update_tid =
    ...
    switch(g_switchapp_data.state)
    {
        ...
        case app_state_disconnecting:
        {
            ...
            else if(p_event_data->reason == HCI_ERROR_CONN_TERM_LOCAL_HOST)
            {
                ...
                if(g_switchapp_data.state == app_state_connected)
                {
...
extern void WriteApplicationAndServiceDataToNVM(void)
{
    ...
    Nvm_Write(g_switchapp_data.assoc_state,
             sizeof(g_switchapp_data.assoc_state),
              NVM_OFFSET_ASSOCIATION_STATE);
...
extern void AppSetState(app_state new_state)
{
     ...
     * of the application.
     */
    app_state old_state = g_switchapp_data.state;
    ...
    if (old_state != new_state)
    {
        ...
        switch (old_state)
        {
            ...
            case app_state_disconnecting:
                ...
                /* Initialise CSR Mesh switch data and services
        ...
        /* Set new state */
        g_switchapp_data.state = new_state;
        ...
        switch (new_state)
        {
            ...
            case app_state_disconnecting:
                GattDisconnectReq(g_switchapp_data.gatt_data.st_ucid);
            break;
...
extern bool AppIsDeviceBonded(void)
{
    ...
    if(g_switchapp_data.gatt_data.bonded)
    {
        if(g_switchapp_data.state == app_state_connected)
        {
            if(!MemCmp(&g_switchapp_data.gatt_data.bonded_bd_addr,
                       &g_switchapp_data.gatt_data.con_bd_addr,
                       sizeof(TYPED_BD_ADDR_T)))
            {
                bonded = TRUE;
...
void AppInit(sleep_state last_sleep_state)
{
    ...
    /* Use static random address for the CSR mesh switch. */
    GapSetStaticAddress();
#endif
...
#else
    /* Initialize the Switch Hardware */
    IOTSwitchInit();
#endif /* DEBUG_ENABLE */
    ...
    /* Initialise the CSR mesh */
    CsrMeshInit();

    /* Enable Relay on Switch */
    if (g_switchapp_data.bearer_data.bearerRelayActive & BLE_BEARER_MASK)
    {
    ...
    /* Initialize Light and Power Model Clients */
    LightModelClientInit();

    PowerModelClientInit();

    /* Initialise the switch Model.*/
    SwitchModelInit(dst_id_list, IOT_NUM_SWITCH_INSTANCES);
    ...
    /* Get the state of ON/OFF button */
    onButtonState = (PioGet(SW4_PIO))?(KEY_RELEASED):(KEY_PRESSED);

    /* Set the CSR Mesh device UUID */
    CsrMeshSetDeviceUUID((CSR_MESH_UUID_T *)switch_uuid);

    /* Set the CSR Mesh device Authorization Code */
    CsrMeshSetAuthorisationCode((CSR_MESH_AUTH_CODE_T *)switchAuthCode);

    /* Tell Security Manager module about the value it needs to initialize it's
     * diversifier to.
     */
    SMInit(g_switchapp_data.gatt_data.diversifier);
    ...
    /* Initialise CSR Mesh switch application State */
    g_switchapp_data.state = app_state_init;
                                   ...
                                   uint16 length, void **state_data)
{
    switch(event_code)
    {
        case CSR_MESH_LIGHT_STATE:
            DEBUG_STR("Received Light State\r\n");
        break;

        case CSR_MESH_POWER_STATE:
        ...
        case CSR_MESH_ASSOCIATION_REQUEST:
        {
            ...
            if (g_switchapp_data.assoc_state != app_state_association_started)
            {
        ...
        case CSR_MESH_KEY_DISTRIBUTION:
        {
            ...
            /* When MESH_BRIDGE_SERVICE is not supported, switch needs to be
             * associated with CSR mesh network, before it can send commands.
             * Stop the blue led blinking visual indication, as switch is
        ...
        case CSR_MESH_GROUP_SET_MODEL_GROUPID:
        {
            uint8 index;
            index = data[1];
            dst_id_list[index] = data[3] + (data[4] << 8);
            
            Nvm_Write(&dst_id_list[index], 1, NVM_OFFSET_DST_ID_LIST + index);
        }
        break;
        ...
        { 
            /* Read Battery Level */
            g_switchapp_data.battery_data.battery_level = ReadBatteryLevel();

            /* Pass Battery state data to model */
            if (state_data != NULL)
            {
                *state_data = (void *)&g_switchapp_data.battery_data;
            }
        }
        break;
        ...
        {
            uint8 *pData = data;
            g_switchapp_data.bearer_data.bearerRelayActive = BufReadUint16(&pData);
            g_switchapp_data.bearer_data.bearerEnabled     = BufReadUint16(&pData);
        ...
        case CSR_MESH_BEARER_GET_STATE:
        {
            if (state_data != NULL)
            {
                *state_data = (void *)&g_switchapp_data.bearer_data;
            }
        }
        break;
        ...
        case CSR_MESH_RAW_MESSAGE:
        {
            if (g_switchapp_data.state == app_state_connected)
            {
                ...
                DEBUG_U16(length);
                DEBUG_STR("\r\n");
                MeshControlNotifyResponse(g_switchapp_data.gatt_data.st_ucid,
                                          data, length);
            }
        }
        break;
