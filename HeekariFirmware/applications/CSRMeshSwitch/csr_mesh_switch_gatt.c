...
static void gattSetAdvertParams(bool fast_connection)
{
    ...
    if((g_switchapp_data.gatt_data.enable_white_list == TRUE))
    {
 ...
 *
 *---------------------------------------------------------------------------*/
static void gattIdleTimerHandler(timer_id tid)
{
    if (g_switchapp_data.gatt_data.idle_tid == tid)
    {
        if (g_switchapp_data.state == app_state_idle)
        {
...
static void gattAdvertTimerHandler(timer_id tid)
{
    ...
    if(g_switchapp_data.gatt_data.app_tid == tid)
    {
        ...
        switch(g_switchapp_data.state)
        {
            case app_state_fast_advertising:
            {
                ...
                /* When device is in fast advertisement mode and
                 * advertisement timeout happens, switch to idle mode for 2
                 * seconds and restart fast advertisements again
                 */

                TimerDelete(g_switchapp_data.gatt_data.idle_tid);

                g_switchapp_data.gatt_data.idle_tid = TimerCreate(
                                              IDLE_TIMEOUT + appRandomDelay(),
                                              TRUE, gattIdleTimerHandler);

                AppSetState(app_state_idle);
            }
            /* FALLTHROUGH */
            case app_state_slow_advertising:
                /* Do not Stop on-going advertisements */
               /* GattStopAdverts(); */
            break;
...
extern void GattTriggerFastAdverts(void)
{
    ...
    /* Do not enable white list as the device can be connected by different 
     * hosts. One for Mesh service and another for OTA firmware update service
     */
    ...
    /* When CSR Mesh is idle, then start connectible Adverts.
     * Other wise wait for next slot. (after IDLE_TIMEOUT).
     */
    if (g_switchapp_data.bearer_data.bearerEnabled & BLE_GATT_SERVER_BEARER_MASK)
    {
        if (CsrMeshStop(FALSE))
        {
    ...
    if(g_switchapp_data.gatt_data.advert_timer_value)
    {
        TimerDelete(g_switchapp_data.gatt_data.app_tid);

        /* Start advertisement timer  */
        g_switchapp_data.gatt_data.app_tid = TimerCreate(
                            g_switchapp_data.gatt_data.advert_timer_value,
