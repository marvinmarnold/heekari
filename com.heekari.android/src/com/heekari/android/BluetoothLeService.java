	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onCharacteristicWrite(final BluetoothGatt gatt,
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				Log.d(TAG, "Characteristic failed");
			}
...
//				intent.putExtra(EXTRA_DATA, Integer.toString(Integer.parseInt(new String(data))));
	 ...
	 */
	public void writePacket(final BluetoothGattCharacteristic characteristic,
			final byte[] buffer, final int size) {
		Log.d(TAG, "Trying to write characteristic");
		...
		try {
			Log.d(TAG, "Trying to write characteristic value: " + (new String(locBuffer, "UTF-8")));
		} catch (UnsupportedEncodingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		mBluetoothGatt.writeCharacteristic(characteristic);
		Log.d(TAG, "Trying to write characteristic finished");
